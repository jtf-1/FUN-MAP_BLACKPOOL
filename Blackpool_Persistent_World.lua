...
function SEF_SaveAirbasesTable(timeloop, time)
	...
	AirbaseStr = IntegratedserializeWithCycles("BlackpoolAirbases", BlackpoolAirbases)
	writemission(AirbaseStr, "BlackpoolAirbases.lua")
	return time + SaveScheduleUnits
end
...
function SEF_PERSISTENTAIRBASES(AirbaseList)
	...
	for i, ab in ipairs(AirbaseList) do
		...
		table.insert(BlackpoolAirbases, TempAirbaseTable )			
	end	
end
...
--////LOAD AIRBASES
if file_exists("BlackpoolAirbases.lua") then

	dofile("BlackpoolAirbases.lua")
	
	AirbaseTableLength = SEF_GetTableLength(BlackpoolAirbases)
	
	for i = 1, AirbaseTableLength do
		BaseName = BlackpoolAirbases[i].Airbase
		BaseCoalition = BlackpoolAirbases[i].Coalition
