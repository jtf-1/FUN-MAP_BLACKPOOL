...
if file_exists("BlackpoolUnitInterment.lua") then
  ...
  for i = 1, UnitIntermentTableLength do
    ...
    if ( Unit.getByName(BlackpoolUnitInterment[i]) ~= nil ) then
      Unit.getByName(BlackpoolUnitInterment[i]):destroy()
      SEFDeletedUnitCount = SEFDeletedUnitCount + 1
    else
      trigger.action.outText("Unit Interment Element "..i.." Is "..BlackpoolUnitInterment[i].." And Was Not Found", 15)
    end 
  end     
else
  DeadUnitsList = SET_UNIT:New():FilterCoalitions("red"):FilterCategories("ground"):FilterActive(true):FilterStart()
...
if file_exists("BlackpoolStaticInterment.lua") then
  ...
  for i = 1, StaticIntermentTableLength do
    --trigger.action.outText("Static Interment Element "..i.." Is "..BlackpoolStaticInterment[i], 15)
    
    if ( StaticObject.getByName(BlackpoolStaticInterment[i]) ~= nil ) then    
      StaticObject.getByName(BlackpoolStaticInterment[i]):destroy()   
      SEFDeletedStaticCount = SEFDeletedStaticCount + 1
    else
      trigger.action.outText("Static Interment Element "..i.." Is "..BlackpoolStaticInterment[i].." And Was Not Found", 15)
    end 
  end 
else
